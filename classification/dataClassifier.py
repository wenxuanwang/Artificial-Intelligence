...
def enhancedFeatureExtractorDigit(datum):
  ...
  "*** YOUR CODE HERE ***"
  whitePixel = util.Counter()
  for feature in features.sortedKeys():
    if features[feature]==0:
        whitePixel[feature]=1
        
  stack = []
  marked = []
  stack.append(whitePixel.keys()[0])
  while len(stack) > 0:
    state = stack.pop(0)
    whitePixel[state] = 0
    if not state in marked:
        marked.append(state)
        x,y = state
        if (x + 1, y) in whitePixel:
            stack.append((x + 1, y))
        if (x - 1, y) in whitePixel:
            stack.append((x - 1, y))
        if (x, y + 1) in whitePixel:
            stack.append((x, y + 1))
        if (x, y - 1) in whitePixel:
            stack.append((x, y - 1))
    features["WhitePixel"] = (1 if whitePixel.totalCount() == 0 else 0) 
  return features

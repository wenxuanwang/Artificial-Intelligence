...
# individual assignment with extra credit implemented
# by Wenxuan Wang 2165909
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for ghostPosition in self.legalPositions:
            ...
            #update
            for newPos,prob in newPosDist.items():
                allPossible[newPos] += prob * self.beliefs[ghostPosition]
        #normalize due to proportional relation        
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for num in range(self.numParticles):
            # use modulo to space evenly
            numOfPos = len(self.legalPositions)
            self.particles.append(self.legalPositions[num % numOfPos])
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()
        # put all particles into the jail if eaten
        if noisyDistance == None:
            ...
            if allPossible.totalCount() == 0:
                ...
                parList = []
                for num in range(self.numParticles):
                    parList.append(util.sample(allPossible))
    ...
    def getBeliefDistribution(self):
        ...
        allPossible = util.Counter()
        #count num particles and normalize
        for particle in self.particles:
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for num in range(self.numGhosts):
            # put into jail if eaten
            if noisyDistances[num] is None:
                dist = util.Counter()
                for particle in distribution:
                    dist[self.getParticleWithGhostInJail(particle, num)] += distribution[particle]
                distribution = dist
            else:
                #update distribution
                for particle in distribution:
                    distance = util.manhattanDistance(particle[num], pacmanPosition)
                    distribution[particle] *= emissionModels[num][distance]
        ...
        if distribution.totalCount() == 0:
            ...
            partiList = []
            for num in range(self.numParticles):
                partiList.append(util.sample(distribution))
            self.particles = partiList
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for num in range(self.numGhosts):
                #new distribution
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),num, self.ghostAgents[num])
                #resample and store back
                newParticle[num] = util.sample(newPosDist)
